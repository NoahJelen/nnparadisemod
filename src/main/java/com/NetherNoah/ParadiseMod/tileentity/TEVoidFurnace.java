...
public class TEVoidFurnace extends TileEntityLockable implements ITickable, ISidedInventory
{
    ...
    {
        return (ItemStack)furnaceItemStacks.get(index);
    }
    /**
    ...
    {
        ItemStack itemstack = (ItemStack)furnaceItemStacks.get(index);
    ...
    public void readFromNBT(NBTTagCompound compound)
    {
        ...
        currentItemBurnTime = getItemBurnTime((ItemStack)furnaceItemStacks.get(1));
    ...
    public void update()
    {
        ...
        if (!world.isRemote)
        {
            ItemStack itemstack = (ItemStack)furnaceItemStacks.get(1);
            if (isBurning() || !itemstack.isEmpty() && !((ItemStack)furnaceItemStacks.get(0)).isEmpty())
            {
                ...
                if (isBurning() && canSmelt())
                {
                    ...
                    {
                        cookTime = 0;
                        totalCookTime = getCookTime((ItemStack)furnaceItemStacks.get(0));
                        smeltItem();
                        flag1 = true;
                    }
                }
    ...
    {
        if (((ItemStack)furnaceItemStacks.get(0)).isEmpty())
        {
            return false;
        }
        else
        {
            ItemStack itemstack = FurnaceRecipes.instance().getSmeltingResult((ItemStack)furnaceItemStacks.get(0));
            if (itemstack.isEmpty())
            {
                return false;
            }
            ...
            {
                ItemStack itemstack1 = (ItemStack)furnaceItemStacks.get(2);
    ...
    public void smeltItem()
    {
        if (canSmelt())
        {
            ...
            ItemStack itemstack2 = (ItemStack)furnaceItemStacks.get(2);
            ...
            if (itemstack.getItem() == Item.getItemFromBlock(Blocks.SPONGE) && itemstack.getMetadata() == 1 && !((ItemStack)furnaceItemStacks.get(1)).isEmpty() && ((ItemStack)furnaceItemStacks.get(1)).getItem() == Items.BUCKET)
            {
