...
public class RoseGen extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos position) {
		...
		Template[] roses= {templatemanager.getTemplate(minecraftserver, new ResourceLocation(Reference.MOD_ID+":rose")),templatemanager.getTemplate(minecraftserver, new ResourceLocation(Reference.MOD_ID+":blue_rose"))};
		
		template=roses[rand.nextInt(2)];

		if(ModConfig.worldgen.GenerateRoses==false)
			return false;

		if(RoseGen.canSpawnHere(template, worldserver, position)) {
