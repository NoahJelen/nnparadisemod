...
public class DimTeleporter extends Teleporter{
    ...
    private final Long2ObjectMap<DimTeleporter.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap(4096);
	public DimTeleporter(WorldServer worldIn,Boolean isDU) {
		super(worldIn);
    ...
    {
        if (worldServerInstance.provider.getDimensionType().getId() != 1)
        {
                ...
                for (int k1 = -2; k1 <= 2; ++k1)
                {
                    for (int l1 = -1; l1 < 3; ++l1)
                    {
                        ...
                        if (DU)
                            ...
                            worldServerInstance.setBlockState(new BlockPos(i2, j2, k2), flag ? Ores.voidPearlBlock.getDefaultState() : Blocks.AIR.getDefaultState());
                    }
                }
            }
	...
	public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_) {
		...
		if (destinationCoordinateCache.containsKey(k)) {
			DimTeleporter.PortalPosition portalposition = destinationCoordinateCache.get(k);
			d0 = 0.0D;
			...
			for (int l = -128; l <= 128; ++l) {
				...
				for (int i1 = -128; i1 <= 128; ++i1) {
					for (BlockPos blockpos = blockpos4.add(l, worldServerInstance.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos
                ...
                for (int l7 = 1; l7 < 3; ++l7)
                {
                    for (int k8 = -1; k8 < 3; ++k8)
                    {
                        ...
                        if (DU)
                        	...
                        	worldServerInstance.setBlockState(new BlockPos(k9, k10, k11), flag ? Ores.voidPearlBlock.getDefaultState() : Blocks.AIR.getDefaultState());
                    }
                }
            }
        }
        ...
        for (int i8 = 0; i8 < 4; ++i8)
        {
            for (int l8 = 0; l8 < 4; ++l8)
            {
                for (int l9 = -1; l9 < 4; ++l9)
                {
                    ...
                    worldServerInstance.setBlockState(new BlockPos(l10, l11, k12), flag1 ? Ores.voidPearlBlock.getDefaultState() : iblockstate, 2);
                    if(DU)
            ...
            for (int i9 = 0; i9 < 4; ++i9)
            {
                for (int i10 = -1; i10 < 4; ++i10)
                {
                    ...
                    worldServerInstance.notifyNeighborsOfStateChange(blockpos, worldServerInstance.getBlockState(blockpos).getBlock(), true);
                }
            }
        }
        return true;
	...
	private boolean func_180265_a(BlockPos pos) {
		return !worldServerInstance.isAirBlock(pos) || !worldServerInstance.isAirBlock(pos.up());
	}
	@Override
	public void removeStalePortalLocations(long worldTime)
    {
        if (worldTime % 100L == 0L)
        {
            ...
            ObjectIterator<DimTeleporter.PortalPosition> objectiterator = destinationCoordinateCache.values().iterator();
