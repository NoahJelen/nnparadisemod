...
public class DVTeleporter extends Teleporter{
	
	private final WorldServer worldServerInstance;
    private final Random random;
    private final Boolean DU;
    //there is a portal already
    public static boolean portalExists=true;
    private final Long2ObjectMap<Teleporter.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap(4096);
	public DVTeleporter(WorldServer worldIn,Boolean isDU) {
		super(worldIn);
	...
	public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_) {
		...
		if (destinationCoordinateCache.containsKey(k)) {
			Teleporter.PortalPosition portalposition = (Teleporter.PortalPosition) destinationCoordinateCache.get(k);
			d0 = 0.0D;
		...
		if (d0 >= 0.0D) {
			if (flag1) {
				destinationCoordinateCache.put(k, new Teleporter.PortalPosition(object, worldServerInstance.getTotalWorldTime()));
			}
			...
			System.out.println("Location:"+Double.toString(d4)+","+Double.toString(d5)+","+Double.toString(d6));
