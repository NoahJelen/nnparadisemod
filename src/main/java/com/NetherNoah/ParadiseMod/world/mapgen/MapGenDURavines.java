                            ...
                            if (d10 * d10 + d7 * d7 < 1.0D){
                            	for (int j2 = l; j2 > l2; --j2){
                                    ...
                                    if ((d10 * d10 + d7 * d7) * (double)this.rs[j2 - 1] + d8 * d8 / 6.0D < 1.0D){
                                    	flag = isTopBlock(primer, j3, j2, i2, p_180707_3_, p_180707_4_);
    ...
    protected void recursiveGenerate(World worldIn, int chunkX, int chunkZ, int originalX, int originalZ, ChunkPrimer chunkPrimerIn){
        if (this.rand.nextInt(50) == 0){
            ...
            for (int j = 0; j < 1; ++j){
                ...
                addTunnel(this.rand.nextLong(), originalX, originalZ, chunkPrimerIn, d0, d1, d2, f2, f, f1, 0, 0, 3.0D);
            }
        }
    }
    
    private boolean isTopBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ){
        return (data.getBlockState(x, y, z).getBlock() == Blocks.GRASS);
    }
