...
public class CaveGenHandler {
	...
	public static void onEvent(ChunkEvent.Load event) {
		...
		// ignore chunks that have already been generated
		if (theChunk.isPopulated()||theChunk.isTerrainPopulated())
			return;
		
		//the height is 256 if the features are being generated in a cave dimension, else, it's 56
		int height=255;
		boolean webs=false;
			...
			for (int z = 0; z < 16; ++z) {
				for (int y = 0; y < 256; ++y) {
					...
					if (jungle||swamp) {
						...
						//vines
						if (blockToReplace == Blocks.STONE&&(jungle||swamp)&&y>=20) {
							// east
							if (theChunk.getBlockState(x + 1, y, z).getBlock() == Blocks.AIR&& rand.nextInt(10) == 0)
							...
							// south
							if (theChunk.getBlockState(x, y, z + 1).getBlock() == Blocks.AIR && rand.nextInt(10) == 0)
								theChunk.setBlockState(new BlockPos(x, y, z + 1),Blocks.VINE.getDefaultState().withProperty(BlockVine.NORTH, true));

							// north
							if ( blockToReplace == Blocks.STONE&& theChunk.getBlockState(x, y, z - 1).getBlock() == Blocks.AIR&& rand.nextInt(10) == 0)
					...
					if (swamp) {
						...
						if(blockToReplace==Blocks.GRASS&&(rand.nextInt(10)==0)&&y<56)
							theChunk.setBlockState(new BlockPos(x, y, z), Blocks.SLIME_BLOCK.getDefaultState());					
						//tall grass
						if(blockToReplace==Blocks.GRASS&&rand.nextInt(5)==0&&y<56)
					...
					if (mushroom) {
						...
						//little mushrooms
						if(blockToReplace==Blocks.MYCELIUM&&y<56&&rand.nextInt(4)==0) {
							BlockPos pos = new BlockPos(x, y+1, z);
							theChunk.setBlockState(pos, Blocks.RED_MUSHROOM.getDefaultState());
						}

						if(blockToReplace==Blocks.MYCELIUM&&y<56&&rand.nextInt(2)==0) {
							BlockPos pos = new BlockPos(x, y+1, z);
							theChunk.setBlockState(pos, Blocks.BROWN_MUSHROOM.getDefaultState());
						}
					...
					//generate lots of crystals
					if(rand.nextInt(2)==0&&crystal&&(blockToReplace == Blocks.STONE || blockToReplace == Blocks.SANDSTONE|| blockToReplace == Blocks.PACKED_ICE || blockToReplace == Blocks.HARDENED_CLAY|| blockToReplace == Blocks.STAINED_HARDENED_CLAY||blockToReplace==Blocks.MYCELIUM||blockToReplace==Blocks.GRASS)&&blockAbove==Blocks.AIR&&y<height) {
