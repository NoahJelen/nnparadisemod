...
public class CustomWall extends Block
{
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos)
    {
        ...
        return BY_INDEX[getAABBIndex(state)];
    }
    @Nullable
    public AxisAlignedBB getCollisionBoundingBox(IBlockState blockState, World worldIn, BlockPos pos)
    {
        blockState = this.getActualState(blockState, worldIn, pos);
        return CLIP_BY_INDEX[getAABBIndex(blockState)];
    }
    @Override
    public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable Entity entityIn, boolean p_185477_7_)
    {
        ...
        addCollisionBoxToList(pos, entityBox, collidingBoxes, CLIP_BY_INDEX[getAABBIndex(state)]);
    }
    ...
    private boolean canConnectTo(IBlockAccess worldIn, BlockPos pos)
    {
        ...
        return block == Blocks.BARRIER || block == this || block instanceof BlockWall || block instanceof CustomWall || block instanceof BlockFenceGate || block instanceof BlockFence || block instanceof RedstoneFenceGate||(block.isOpaqueCube(iblockstate) && iblockstate.isFullCube());
    }
    @Override
