...
public class DVPortal_orig extends Block{
	...
	public boolean trySpawnPortal(World worldIn, BlockPos pos) {
		DVPortal_orig.Size blockportal$size = new DVPortal_orig.Size(worldIn, pos, EnumFacing.Axis.X);	
		if(blockportal$size.isValid() && blockportal$size.portalBlockCount == 0) {
			blockportal$size.placePortalBlocks();
			return true;
		}
		else {
			DVPortal_orig.Size blockportal$size1 = new DVPortal_orig.Size(worldIn, pos, EnumFacing.Axis.Z);
	...
	public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn) {
		...
		if(enumfacing$axis == EnumFacing.Axis.X) {
			...
			DVPortal_orig.Size blockportal$size = new DVPortal_orig.Size(worldIn, pos, EnumFacing.Axis.Z);
			if(!blockportal$size.isValid() || blockportal$size.portalBlockCount < blockportal$size.width * blockportal$size.height) {
				worldIn.setBlockState(pos, Blocks.AIR.getDefaultState());
			}
		}
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		if(!entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn.isNonBoss() && entityIn instanceof EntityPlayerMP) {
			...
			if(thePlayer.timeUntilPortal > 0) {
				...
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, DimensionRegistry.DeepVoid, new DVTeleporter(thePlayer.mcServer.getWorld(DimensionRegistry.DeepVoid),false));
				worldIn = thePlayer.getEntityWorld();
				Random rand=new Random();
				for (int j=-10;j<=10;j++) {
					for (int i=-10;i<=10;i++) {
						if (rand.nextInt(2)==1)
							worldIn.setBlockState(new BlockPos(thePlayer.posX + i, thePlayer.posY - 4, thePlayer.posZ + j), ModBlocks.VoidStone.getDefaultState());
						else
				...
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, 0, new DVTeleporter(thePlayer.mcServer.getWorld(0),false));
			}
		}
	}
